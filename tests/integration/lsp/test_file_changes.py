...
class TestFileChangesPython:
    ...
    async def test_notify_created_file_then_open(
        ...
        new_file = python_workspace / "mock_pkg" / "new_module.py"
        new_file.write_text(
        ...
        async with python_client.open_files(new_file):
            symbols = await python_client.request_document_symbol_list(new_file)
    ...
    async def test_notify_deleted_file(
        ...
        new_file = python_workspace / "mock_pkg" / "to_delete.py"
        ...
        existing = python_workspace / "mock_pkg" / "utils.py"
        async with python_client.open_files(existing):
            symbols = await python_client.request_document_symbol_list(existing)
        assert symbols is not None
    ...
    async def test_notify_batch_events(
        ...
        file_a = python_workspace / "mock_pkg" / "batch_a.py"
        file_b = python_workspace / "mock_pkg" / "batch_b.py"
        file_a.write_text("def func_a(): pass\n", encoding="utf-8")
        file_b.write_text("def func_b(): pass\n", encoding="utf-8")

        await python_client.notify_did_change_watched_files_raw(
            [
                _file_event(file_a.as_uri(), lsp_type.FileChangeType.Created),
                _file_event(file_b.as_uri(), lsp_type.FileChangeType.Created),
            ]
        )

        async with python_client.open_files(file_a):
            symbols_a = await python_client.request_document_symbol_list(file_a)
        async with python_client.open_files(file_b):
            symbols_b = await python_client.request_document_symbol_list(file_b)

        assert symbols_a is not None and "func_a" in {s.name for s in symbols_a}
        assert symbols_b is not None and "func_b" in {s.name for s in symbols_b}
